/**
 * TODO:
 * This is a patched version of @auth/sequelize-adapter
 * Using this until PR is merged: https://github.com/nextauthjs/next-auth/pull/13120
 */
...
export default function SequelizeAdapter(
  ...
  const { User, Account, Session, VerificationToken } = {
    User:
      models?.User ?? client.define<UserInstance>('user', defaultModels.User, defaultModelOptions),
    Account:
      models?.Account ??
      client.define<AccountInstance>('account', defaultModels.Account, defaultModelOptions),
    Session:
      models?.Session ??
      client.define<SessionInstance>('session', defaultModels.Session, defaultModelOptions),
    VerificationToken:
